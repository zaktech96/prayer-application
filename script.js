...
navigator.geolocation.getCurrentPosition((position) => {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      const maghribMinutes = data.data[0].timings.Maghrib.slice(0, 5).split(
        ":"
      )[1];
      ...
      if (
        currentHours >= dhuhrHours ||
        (currentHours === dhuhrHours && currentMinutes >= dhuhrMinutes)
      ) {
        nextPrayer = "Dhuhr";
        return `The next prayer is ${nextPrayer} which is ${timeLeft(
          dhuhrHours,
          dhuhrMinutes
        )}`;
      } else if (
        currentHours >= asrHours ||
        (currentHours === asrHours && currentMinutes >= asrMinutes)
      ) {
        nextPrayer = "Asr";
        return `The next prayer is ${nextPrayer} which is ${timeLeft(
          asrHours,
        ...
        currentHours >= maghribHours ||
        (currentHours === maghribHours && currentMinutes >= maghribMinutes)
      ) {
        nextPrayer = "Maghrib";
        return `The next prayer is ${nextPrayer} which is ${timeLeft(
          maghribHours,
        ...
        currentHours >= ishaHours ||
        (currentHours === ishaHours && currentMinutes >= ishaMinutes)
      ) {
        nextPrayer = "Isha";
        return `The next prayer is ${nextPrayer} which is ${timeLeft(
          ishaHours,
        ...
        return (timing.innerHTML = `The next prayer is ${nextPrayer} which is ${timeLeft(
          fajrHours,
          fajrMinutes
        )}`);
      }
