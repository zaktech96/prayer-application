...
navigator.geolocation.getCurrentPosition((position) => {
  ...
  fetch(
    `https://api.aladhan.com/v1/calendar?latitude=${lat}&longitude=${long}&method=15&month=${currentMonth}&year=${currentYear}`
  )
    .then((res) => {
      if (!res.ok) {
...
checkboxes.forEach((checkbox, index) => {
  ...
  if (localStorage.getItem(`checkbox${index + 1}Value`)) {
    ...
    checkbox.checked = JSON.parse(
      localStorage.getItem(`checkbox${index + 1}Value`)
    );
  }
...
fetch("quotes.json")
  ...
  .then((data) => {
    ...
    document.querySelector(
      ".quote-ref"
    ).textContent = `${data.quotes[0].author}`;
  });
